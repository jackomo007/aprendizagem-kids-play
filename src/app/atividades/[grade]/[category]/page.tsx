...
import { activityCategoryParams, getActivityCategory, getActivityGrade } from "@/data/activity-navigation";
...
export default async function ActivityCategoryPage({ params }: { params: Promise<{ grade: string; category: string }> }) {
  ...
  const filteredActivities = activities.filter(
    (activity) => activity.gradeSlugs.includes(grade.slug) && activity.categorySlug === category.slug,
  );
  return (
    <Container className="page-container activity-results-page">
      <nav className="activity-path activity-path-top" aria-label="Caminho das atividades">
        ...
        <Link href={`/atividades/${grade.slug}/`}>{grade.title}</Link><span aria-hidden="true">›</span>
      ...
      <section className="section compact-section" aria-labelledby="activities-heading">
        <header className="activity-results-header">
          ...
          <p>{grade.title} · Escolha uma atividade para visualizar ou baixar.</p>
        </header>
        {filteredActivities.length ? (
          <>
            <div className="content-grid">
              {filteredActivities.map((activity) => <ActivityCard key={activity.slug} activity={activity} levelOverride={grade.title} />)}
            </div>
          ...
          <div className="activity-empty">
            ...
            <p>Ainda não há materiais de {category.title} para {grade.title}. Volte em breve ou escolha outra opção.</p>
            <Link className="button button-secondary" href={`/atividades/${grade.slug}/`}><Icon name="arrow" />Ver outras opções do {grade.title}</Link>
          </div>
        )}
      </section>
    </Container>
